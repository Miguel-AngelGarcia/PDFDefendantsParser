...
def error_file_write(file_write):
    with open('SocialMedia09232022.csv', 'a+', newline='') as result_file:
...
files = [file for file in glob.glob("/Users/miguelgarcia/Desktop/Work/LitigationTracking/September-October/COVID/Edit/*")]
...
for file_name in files:
    ...
    try:
        ...
        # print(type(pdf_file))
        reader = PdfFileReader(file_string)
    ...
    for defendant in defendants:
        ...
        for fka in fka_dict:
            if re.search(fka, defendant, flags=re.IGNORECASE):  # do some regex magiv for fka, f/k/a, FKA, F/K/A, etc
        ...
        for successor in successor_dict:
            if re.search(successor, defendant, flags=re.IGNORECASE):
    ...
    with open('SocialMedia09232022.csv', 'a+', newline='', encoding='utf-8') as result_file:
        wr = csv.writer(result_file)  # , dialect='excel')
